...
for(let i=0;i<LEVEL_COUNT;i=i+1){
    levelButtons[i]=update_position(create_rectangle(100,60),[-500,500]);
}
const ChallengeButtons=update_position(create_rectangle(100,60),[-500,500]);
let ChallengeTexts=update_position(create_text(""),[-500,-500]);
let ChallengeTime=0;
let InitChallengeTime=0;
let Challenge=false;
...
const level5_hs = [120,204,204,190,204,204,150,204,204,160,204,204,204];
...
function initSelect() {
    ...
    while (i < LEVEL_COUNT) {
        ...
        update_color(levelButtons[i], [102,51,0,255]); // ensure white
        // 创建关卡文本
    ...
    update_position(update_color(ChallengeButtons,[102,51,0,255]),[500,400]);
    update_position(update_color(update_text(ChallengeTexts,"Challenge"),[255,255,255,255]),[500,400]);
...
function clearselect(){
    ...
    update_position(ChallengeButtons,[-500,-500]);
    update_position(ChallengeTexts,[-500,-500]);
...
update_loop(game_state => {
    ...
    curtainsequence();
    debug_log("get_game_time: "+stringify(get_game_time()));
    debug_log("InitChallengeTime: "+stringify(InitChallengeTime));
    for (let i = 0; i < array_length(player_frames); i = i + 1) {
        debug_log("player_frames[" + i + "] pos: " + stringify(query_position(player_frames[i])));
    }
    for (let i = 0; i < array_length(level6_player_frames); i = i + 1) {
    ...
    if(currentState===6.5){
        ...
        if(Challenge){
            if(ChallengeTime===0){
                ChallengeTime=get_game_time()-InitChallengeTime;
            }
            if(input_key_down("b") && curtainSequence === 3 && !pendingTransition){
                curtainSequence = 0;
    ...
    if(currentState===6){
        ...
        if(input_key_down("b") && curtainSequence === 3 && !pendingTransition && !Challenge){
            curtainSequence = 0;
        ...
        if(level6_alive && !door_anim_active){
            ...
            if (level6_gear_active && gameobjects_overlap(level6_current_player, level6_gear))
            {
                ...
                if(Challenge){
                    update_color(update_text(level6_text, "Game Over!"),[255,255,255,255]);
                }
    ...
    if(currentState===5.5){
        if(Challenge){
            if(curtainSequence === 3 && !pendingTransition){
                curtainSequence = 0;
    ...
    if(currentState===4.5){
        if(Challenge){
            ...
            if(input_key_down("b") && curtainSequence === 3 && !pendingTransition){
                curtainSequence = 0;
                pendingTransition = "main_from_4";
            }
            if(input_key_down("n") && curtainSequence === 3 && !pendingTransition){
                curtainSequence = 0;
                pendingTransition = "next4";
            }
            if(curtainSequence === 3 && pendingTransition === "main_from_4"){
                currentState="select";
                pendingTransition = null;
            }
            if(curtainSequence === 3 && pendingTransition === "next4"){
            ...
            if(curtainSequence === 2 && pendingTransition === "main_from_4"){
                clearlevel4();
                initSelect();
            }
            if(curtainSequence === 2 && pendingTransition === "next4"){
    ...
    if(currentState===2.5){
        ...
        if(Challenge){
            if(curtainSequence === 3 && !pendingTransition){
                curtainSequence = 0;
                pendingTransition = "next2";
            }
            if(curtainSequence === 3 && pendingTransition === "next2"){
                currentState=3;
                pendingTransition = null;
            }
            if(curtainSequence === 2 && pendingTransition === "next2"){
                cleartutorial();
                clearlevel2(); 
                clear_death_pieces();
                level3setup();
    ...
    if(currentState==="select"){
        ...
        if(curtainSequence===2){
            ...
            if(k===7){
                clearselect();
                level1setup();
                InitChallengeTime=get_game_time();
                ChallengeTime=0;
                Challenge=true;
                currentState=1;
                k=-1;
    ...
    if (door_anim_active) {
        ...
        if(currentState===6){
            ...
            if (door_anim_y >= 350) {
                ...
                if(Challenge){
                    update_position(update_text(collision_happened, "Challenge Time: "+stringify(ChallengeTime)),[500,300]);
                }
                victorySound();
                currentState = door_anim_nextState;
            }
